  ...
  topics: topics
      ...
      },
      {
        parameters: {
          ...
          sendBody: true,
          specifyBody: "json",
          jsonBody: `={\n  "chat_id": "${telegramChatId}",\n  "text": "📢 <b>[테스트] 1차 콘텐츠 가대본 기획 완료</b>\\n\\n📌 <b>주제</b>: {{ $('Mock Parser Data').first().json.topic }}\\n⭐ <b>AI 검수 점수</b>: {{ $('Mock Parser Data').first().json.score }}/10점\\n\\n📝 <b>기획 요약</b>:\\n{{ $('Mock Parser Data').first().json.script_summary }}\\n\\n📁 <a href=\\"https://docs.google.com/spreadsheets/d/1kW7YKfXqccDz3GEqDY_zLt5lqcnHHE6B3jum96yerX0/edit#gid=0\\"><b>구글 시트에서 전체 가대본 보기</b></a>\\n\\n위 가대본으로 다음 단계(최종 대본 및 영상 리소스 제작)를 진행할까요?",\n  "parse_mode": "HTML",\n  "reply_markup": {\n    "inline_keyboard": [\n      [\n        { "text": "👍 승인", "callback_data": "=approve_1_{{ $('Mock Parser Data').first().json.row_id }}" },\n        { "text": "👎 반려", "callback_data": "=reject_1_{{ $('Mock Parser Data').first().json.row_id }}" }\n      ],\n      [\n        { "text": "🔄 내용 보강 필요 재작업", "callback_data": "=retry_1_{{ $('Mock Parser Data').first().json.row_id }}" }\n      ]\n    ]\n  }\n}`,
          options: {}
        },
